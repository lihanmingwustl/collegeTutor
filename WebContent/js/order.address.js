...
$(function(){
	...
	$("#receiveName").blur(function(){
		...
		info.text("");
		if(val == ""){
			info.text("Recipient cannot be empty");
		}else{
	...
	$("#fullAddress").blur(function(){
		...
		info.text("");
		if(val == ""){
			info.text("The destination address cannot be empty");
		}else{
			info.html("<img src='../images/my/ajax_ok.png' />");
			A = true;
		}
	...
	$("#postalCode").blur(function(){
		...
		info.text("");
		if(val == ""){
			info.text("Zip code can not be empty");
		}else if (!val.match(/^[0-9]{6,6}$/)) {
			info.text("The zip code is not in the correct format");}else{
			info.html("<img src='../images/my/ajax_ok.png' />");
			P = true;
		}
	});
